...
func (b *Builder) AddSection(name string, data []byte) {
	fmt.Printf("Adding section %s: size=%d\n", name, len(data))
	b.pe.AddSection(name, uint32(len(data)))

	// TODO: there's something wrong here
	index := len(b.pe.ImageSectionHeaders) - 1
	offset := b.pe.ImageSectionHeaders[index].PointerToRawData
	fmt.Printf("index=%d, offset=%d\n", index, offset)
	copy(data, b.pe.Raw[offset:])
    //b.newSections = append(b.newSections, Section{Name: name, Reader: reader})
}
